$(document).ready(function () {
  ...
  function getPopDonations() {
    $.ajax({
      url: 'functions/process.php', // Replace with your API endpoint
      method: 'GET',
      data: {'get_donations': 'get'},
      success: function(data) {
        if (data.trim() === '') return false;
        $('#popDonation').fadeOut(500, function() {
            $(this).html(data).fadeIn(500);
        });
      },
      error: function() {
        $('#popDonation').fadeOut(500, function() {
            $(this).html('Error fetching data').fadeIn(500);
            clearInterval();
        });
      }
    });
   ...
   // Initial call
  getPopDonations();
  // Set interval to fetch content every 1 second
  setInterval(getPopDonations, 5000);
